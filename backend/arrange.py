...
def postprocess_segmentation(predicted_mask):
...
def generate_mask(model, image_path, output_filename):
    ...
    if predicted_mask.shape[-1] == 2:
        binary_mask = predicted_mask[0, :, :, 1]  # Foreground
    else:
        binary_mask = predicted_mask[0, :, :, 0]

    binary_mask = postprocess_segmentation(binary_mask)
    binary_mask_uint8 = (binary_mask * 255).astype(np.uint8)

    cv2.imwrite(output_filename, binary_mask_uint8)
